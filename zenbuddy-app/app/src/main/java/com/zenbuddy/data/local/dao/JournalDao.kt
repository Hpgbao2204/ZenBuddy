...
interface JournalDao {
    ...
    suspend fun getUnsynced(): List<JournalEntity>

    @Query("UPDATE journals SET isSynced = 1 WHERE id IN (:ids)")
    suspend fun markSynced(ids: List<String>)

    @Query("SELECT * FROM journals ORDER BY createdAt DESC LIMIT :limit")
    suspend fun getRecent(limit: Int = 3): List<JournalEntity>
}
